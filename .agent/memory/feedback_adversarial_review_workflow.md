...
**Exit condition:** If Codex repeats a finding across 2+ rounds and the underlying constraint is external (e.g., a third-party API limitation that cannot be designed around), stop iterating. Put the residual-risk decision to the user instead of grinding. The options are: accept with documented risk, descope the capability, or block until the external constraint is resolved.
